...
function jsonToGo(json:string, typename?: string, flatten: boolean = true)
{
	let data;
	let scope;
	let go = "";
	...
	function parseScope(scope: any, depth: number = 0)
	{
	...
	function toProperCase(str: string)
	{
		...
		return str.replace(/(^|[^a-zA-Z])([a-z]+)/g, function(unused: string, sep?: any, frag?: any)
		{
			if (commonInitialisms.indexOf(frag.toUpperCase()) >= 0)
				return sep + frag.toUpperCase();
			else
				return sep + frag[0].toUpperCase() + frag.substr(1).toLowerCase();
		}).replace(/([A-Z])([a-z]+)/g, function(unused: string, sep?: any, frag?: any)
		{
	...
	function formatScopeKeys(keys: Array<string>) {
		for (let i in keys) {
